...
def calcul_ratio(list_dep):
    ...
    data_immat_dep=data_immat_dep.groupby(['dptmt']).sum()
    data_bornes_dep=data_bornes[data_bornes['dep_b'].isin(list_dep)]
    data_bornes_dep=data_bornes_dep.groupby(['dep_b']).sum()
    data_bornes_dep['ratio']=data_immat_dep['Nb_immat']/data_bornes_dep['Nombre points de charge']
    return data_bornes_dep['ratio']

def repart_energie(annee):
    data = data_immat[data_immat['Annee']==annee]
    data = data.groupby(['energie']).sum()
    return data['Nb_immat']

def repart_ratio():
    # Filtrer les communes avec une population différente de zéro
    data_communes_filtered = data_communes[data_communes['POPU'] != 0]

    # Calculer le nombre total d'immatriculations par commune
    data_immat_filtered = data_immat.groupby('code commune')['Nb_immat'].sum().reset_index()
    data_merged = pd.merge(data_communes_filtered, data_immat_filtered, left_on='COG', right_on='code commune')
    data = data_merged[['Nb_immat', 'POPU', 'ADMI']]
    data['ratio'] = data['Nb_immat'].astype(int) / data['POPU']*10/13
    data = data[data['ratio']<100][['ratio','ADMI','Nb_immat','POPU']]
    
    # Calculer le ratio de statut administratif sur toutes les communes
    data_repart_admi = data.groupby('ADMI')['Nb_immat'].sum()
    data_repart_popu = data.groupby('ADMI')['POPU'].sum()
    data_repart = data_repart_admi / data_repart_popu*10/13
...
print(serie_ratio_com.mean(),19.887652614523343)
print('test_repart_ratio2 : ',np.isclose(serie_ratio_admi.loc[3],34.55047603236587))
print((serie_ratio_admi.loc[3],34.55047603236587))
